...
export function useApiData<
  TData,
  TError = Error,
  TFetcher extends (params?: any) => Promise<{ data?: TData; error?: TError }> = any
>(
  fetcher: TFetcher,
  options: UseApiDataOptions<ExtractParams<TFetcher>> = {}
) {
  ...
  const fetchData = useCallback(async (fetchParams?: TParams) => {
    // Skip if disabled
    if (!enabled) return
  ...
  const refetch = useCallback((newParams?: TParams) => {
